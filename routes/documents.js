...
router.put('/:id/replace', upload.single('document'), async (req, res) => {
  try {
    ...
    const newDocumentData = {
      nomdoc: req.file.originalname,
      type: req.file.mimetype.includes('pdf') ? 'pdf' : 'image',
      nom_fichier: req.file.filename,
      statut: 'en_attente',
    };

    const updatedDocument = await Document.updateStatus(id, newDocumentData.statut, 'Document remplacé pour revalidation');

    const dossier = await Dossier.findById(existingDocument.dossier_id);
    if (dossier) {
      await Dossier.update(dossier.id, { docdsr: req.file.path });
    }
