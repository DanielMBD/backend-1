...
router.put('/:id', async (req, res) => {
  try {
    ...
    try {
      const emailResult = await emailService.sendDocumentValidationNotification(document, statut, commentaire);
      console.log('Résultat envoi email de validation:', emailResult);
    } catch (emailError) {
      console.error('Erreur envoi email de validation (non bloquant):', emailError);
