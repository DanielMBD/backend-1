...
class EmailService {
  ...
  async sendAdminCredentials(adminData) {
    try {
      ...
      const mailOptions = {
        from: process.env.FROM_EMAIL || 'noreply@concours.ga',
        to: adminData.email,
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p>Vous pouvez vous connecter à l'adresse : <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/admin/login">Connexion Admin</a></p>
  ...
  async sendReceiptEmail(data) {
    try {
      console.log('Envoi reçu email à:', data.to);
      
      const mailOptions = {
        from: process.env.FROM_EMAIL || 'noreply@concours.ga',
        to: data.to,
        subject: `Reçu de candidature - ${data.nupcan}`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #2563eb;">Reçu de candidature</h2>
            
            <p>Bonjour ${data.candidatData.candidat.prncan} ${data.candidatData.candidat.nomcan},</p>
            
            <p>Votre candidature a été enregistrée avec succès.</p>
            
            <div style="background: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
              <p><strong>NUPCAN :</strong> ${data.nupcan}</p>
              <p><strong>Concours :</strong> ${data.candidatData.concours.libcnc}</p>
  ...
  async sendReceiptImageEmail(data) {
    try {
      console.log('Envoi reçu image email à:', data.to);
  ...
  async sendDocumentValidationEmail(data) {
    try {
      console.log('Envoi notification validation à:', data.to);
